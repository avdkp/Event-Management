...
if(isset($_POST["submit"]))
{
	...
	if($link)
	{
		...
		$check = mysqli_query($link,"select count(*) from events where eid = '$eid'");
		...
		if($exist[0]==0)
		{
			...
			if(!$start || !$s)
			{
				...
				$q1 = "insert into events (ename,edate,etime,evenue,edesc,max_reg) values('$ename','$edate','$etime','$evenue','$message','$max_reg')";
				...
				for($j=1;$j<=$number;$j++)
				{
					...
					$q2 = mysqli_query($link,"insert into rules (ename,rule) values('$ename','$rule')");
					if(!$q2)
					{
						mysqli_query($link,"ROLLBACK");
						echo "<script>alert('Rule insertion failed ..rolling back...');</script>";
						exit();
					}
					
				}
			mysqli_query($link,"COMMIT");
			echo "<script>alert('Event Registered Successfully...".$ename."');</script>";
			mysqli_close($link);
		}
		else if($exist[0]==1)
		{
			mysqli_query($link,"BEGIN");		//Transaction starts
				...
				$q2 = "delete from rules where ename=(select distinct ename from events where eid='$eid')";
				$q1 = "update events set ename='$ename',edate='$edate',etime='$etime',evenue='$evenue',edesc='$message',max_reg='$max_reg' where eid='$eid'";
				...
				for($i=1;$i<=$number;$i++)
				{
					...
					{
						mysqli_query($link,"ROLLBACK");
						echo "<script>alert('Rule updation failed ..rolling back...');</script>";
						exit();
					}
				}
			mysqli_query($link,"COMMIT");
	...
	<?php
		...
		{	
			...
			if($link)
			{
				...
				$res = mysqli_query($link,"select * from events where ename='$event' or eid='$eid'");
				$ans = mysqli_fetch_assoc($res);
				$ename = $ans['ename'];
				
				if($eid>0)
					$q1 = "select rule from rules where ename=(select ename from events where eid='$eid') order by no";
				else
					$q1 = "select rule from rules where ename='$ename' order by no";
				...
				if($res2)
				{
					...
					{
						echo 'container.appendChild(document.createTextNode("Rule " +'.$i.'));
		...
		<div class="gtco-container">
			<div class="row">
				<div class="col-md-12 text-right gtco-contact">
							<form class="form-inline" action="e_in.php" method="post" style="display:none;" id="up_form">
								  ...
								  <div class="form-group">
									<label for="pwd">or Event ID:</label>
			...
			<div class="row" id="menu">
				<div class="col-sm-4 col-xs-12">
					<div id="gtco-logo"><a href="index.html">DBMS <em>Register Events</em></a></div>
				</div>
				<div class="col-xs-8 text-right menu-1">
					<ul>
						...
						<li id="update1"><a href="#">Update</a></li>
				...
				<div class="col-md-8 col-md-offset-2 text-center">
					<div class="display-t">
						<div class="display-tc">
							...
							<h2 class="animate-box" data-animate-effect="fadeInUp">Event Registration </h2>
						</div>
					</div>
				</div>
			</div>
		...
		<div class="gtco-container">
			...
			<div class="col-md-3 animate-box">
				...
				<div class="col-md-6 animate-box">
					<h3>Please fill the details about event</h3>
					<form action="e_in.php" method="post">
						<div class="row form-group">
							<div class="col-md-12">
								<input type="hidden" id="eid" name="eid" class="form-control" value="<?php if(isset($ans['eid']))echo $ans['eid'];  ?>" placeholder="Name of Event" readonly>
							</div>
							
						</div>
						<div class="row form-group">
							<div class="col-md-12">
								<label class="sr-only" for="name">Name of Event</label>
								<input type="text" id="name" name="ename" class="form-control" value="<?php if(isset($ans['ename'])) echo $ans['ename'];  ?>" placeholder="Name of Event">
							</div>
						...
						<div class="row form-group">
							<div class="col-md-12">
								...
								<input type="text" id="name" name="evenue" class="form-control" value="<?php if(isset($ans['evenue'])) echo $ans['evenue'];  ?>" placeholder="Venue">
							</div>
						...
						<div class="row form-group">
							<div class="col-md-12">
								...
								<input type="date" id="name" name="edate" class="form-control" value="<?php if(isset($ans['edate'])) echo $ans['edate'];  ?>" placeholder="Date">
							</div>
						...
						<div class="row form-group">
							<div class="col-md-12">
								...
								<input type="time" id="name" name="etime" class="form-control" value="<?php if(isset($ans['etime'])) echo $ans['etime'];  ?>" placeholder="time">
							</div>
						...
						<div class="row form-group">
							<div class="col-md-12">
								...
								<textarea name="msg" id="message" cols="30" rows="7" class="form-control" placeholder="Description"> <?php if(isset($ans['edesc'])) echo $ans['edesc'];  ?></textarea>
							</div>
						</div>
						...
						<div class="form-group">
							<input type="button" class="btn" id="rules_btn" value="Add Rule" onclick="addFields()" >
							<input type="button" class="btn" id="rules_btn1" value="Remove Rule" onclick="removeFields()"><br>
    ...
    <script type='text/javascript'>
        function addFields(){
			...
			document.getElementById("no_of_rules").value = n;
                ...
                container.appendChild(document.createTextNode("Rule " + n));
